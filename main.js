...
const linkedByIndex = [];
const { colorAccessor, nodeRadius } = scales;
...
function networkLayout({ ...data }) {
  ...
  d3.select('#title h2')
    .html(`${nodes.length} entities, ${links.length} edges`)

  d3.select('#summary')
    .html(`${articles_count.length} ARTICLES, ${categories_count.length} CATEGORIES, ${keywords_count.length} KEYWORDS`)

  d3.select('#top_categories').select('ul').selectAll('li')
    .data(categories_count.slice(0,3))
    .enter()
    .append('li')
    .text(d=>d.id)

  d3.select('#top_keywords').select('ul').selectAll('li')
  ...
  function draw(nodes, links, accessors, summary) {
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    d3.select('.fold-button-div')
      .on('click', function(){
        clicked = !clicked
        d3.select('.abstractbox-column')
          .classed('folded', clicked)
        d3.select('.fa-caret-right')
          .style('display', clicked ? 'none' : 'block')
        d3.select('.fa-caret-left')
          .style('display', clicked ? 'block' : 'none')
        d3.select('.networkWrapper')
          .attr('width', clicked ? screen.width : screen.width - 360)
      })
  ...
  function updateAttributes(nodes, links) {
    const ROOT_IDs = nodes
      .filter((d) => d.category === "author")
      .map((d) => d.id);
    ...
    var distanceScale = d3.scaleOrdinal()
      .domain(["author-keywords", "author-categories", "keywords-categories"])
      .range([15, 50, 15])
    ...
    links.forEach((d) => {
      ...
      d.distance = distanceScale(d.category)
  ...
  function updateGraph(data) {
    ...
    links = links.filter((d) => {
      return (
        (nodeIDs.indexOf(d.start_id) !== -1) &
        (nodeIDs.indexOf(d.end_id) !== -1)
      );
    });
    ...
    var forceCollide = d3.forceCollide(function (d) {
      return Math.max(d.radius*1.5, 25)
      //return d.radius * 1.9;
    });
    ...
    simulation
      ...
      .force(
        "center",
        d3.forceCenter(graphWrapper.width / 2, graphWrapper.height / 2)
      );
...
function showTooltip(data, links, summary){
  ...
  const top_categories = top_list_data.filter(d=>d.category === 'author-categories')
  ...
  d3.select('#summary')
    .html(`${summary_data.article_count} articles written, with total of ${summary_data.comment_count} comments written by readers.`)

  d3.select('#top_categories').select('ul').selectAll('li')
    .data(top_categories.slice(0,4))
    .enter()
    .append('li')
    .text(d=>d.end_id)

  d3.select('#top_keywords').select('ul').selectAll('li')
    .data(top_keywords.slice(0,4))
    .enter()
    .append('li')
    .text(d=>d.end_id)
