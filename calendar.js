function calendar() {
  d3.selectAll("#calendar-chart svg").remove()
  ...
  const renderCalendar = (name) => {
    d3.csv("/data/timeline.csv").then(function (data) {
      ...
      authors.forEach((d) => {
        ...
        days.forEach((x) => {
          let t = data.find(
            (el) => (el.day_month.getTime() === x.getTime()) & (el.author === d[0])
          )
          if (t) {
            empty.push({
              day_month: new Date(x.setDate(x.getDate() + 1)),
              value: t.value,
              author: d[0],
            })
          } else {
      ...
      year
        ...
        .data(
          weekday === "weekday"
            ? ([, values]) =>
                values.filter((d) => ![0, 6].includes(d.day_month.getUTCDay()))
            : ([, values]) => values
        )
        .join("rect")
